...
class Event extends Controller
{
    public function index()
    {
        ...
        return View('admin/page/destinasi_event', compact('title'));
    }

    public function getData()
    {
        $cat = CategoryPosts::where('name', 'Event')->first()->id;
        $event = Articles::where('category_id', $cat)->orderBy('id', 'asc')->get();
        $data['data'] = $event->map(function($d){
            return [
                'id' => $d->id,
                'gambar' => $d->image,
    ...
    public function create(Request $request)
    {
        ...
        */
        ...
        $upload = Articles::create([
            ...
            'category_id' => CategoryPosts::where('name', 'Event')->first()->id
        ]);
        //$this->logActivity("Mengunggah dokumen regulasi dengan judul ".$upload->title);

        return redirect()->route('admin_destinasi_event_index')
            ->with([
                'status' => 'success',
                'title' => 'Berhasil Menambah Event',
                'message' => 'Event '.$judul.' Berhasil Diunggah'
            ]);
    }
